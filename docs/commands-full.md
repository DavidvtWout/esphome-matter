...
matter.groups.get_group_membership:
  group_list:
...
matter.groups.remove_all_groups:
...
matter.groups.add_group_if_identifying:
  group_id:
  group_name:
```
...
matter.on_off.on_with_recall_global_scene:
...
matter.access_control.review_fabric_restrictions:
  arl:
```
...
matter.ota_software_update_provider.query_image:
  vendor_id:
  product_id:
  software_version:
...
matter.ota_software_update_provider.apply_update_request:
  update_token:
  new_version:

# This command tells the Provider that the specified update has been applied.
matter.ota_software_update_provider.notify_update_applied:
  update_token:
  software_version:
```
...
matter.ota_software_update_requestor.announce_ota_provider:
  provider_node_id:
  vendor_id:
...
matter.diagnostic_logs.retrieve_logs_request:
...
matter.general_diagnostics.test_event_trigger:
  enable_key:
  event_trigger:
...
matter.general_diagnostics.time_snapshot:
...
matter.general_diagnostics.payload_test_request:
  enable_key:
  value:
  count:
```
...
matter.software_diagnostics.reset_watermarks:
```
...
matter.thread_network_diagnostics.reset_counts:
```
...
matter.wi_fi_network_diagnostics.reset_counts:
```
...
```yaml
# This command is used to reset the count attributes.
matter.ethernet_network_diagnostics.reset_counts:
```
...
matter.bridged_device_basic_information.keep_active:
...
matter.administrator_commissioning.open_commissioning_window:
...
matter.administrator_commissioning.open_basic_commissioning_window:
  commissioning_timeout:
...
matter.administrator_commissioning.revoke_commissioning:
```
...
matter.operational_credentials.set_vid_verification_statement:
  # vendor_id:
  # vid_verification_statement:
  # vvsc:

# This command is used to authenticate the fabric associated with the FabricIndex.
matter.operational_credentials.sign_vid_verification_request:
  fabric_index:
  client_challenge:
```
...
matter.icd_management.register_client:
  check_in_node_id:
  monitored_subject:
  key:
  # verification_key:
...
matter.icd_management.unregister_client:
  check_in_node_id:
  # verification_key:
...
matter.icd_management.stay_active_request:
  stay_active_duration:
```
...
matter.oven_mode.change_to_mode:
  new_mode:
```
...
matter.mode_select.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.laundry_washer_mode.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.refrigerator_and_temperature_controlled_cabinet_mode.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.rvc_run_mode.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.rvc_clean_mode.change_to_mode:
  new_mode:
```
...
matter.temperature_control.set_temperature:
  # target_temperature:
...
```yaml
# This command is used to change device modes.
matter.dishwasher_mode.change_to_mode:
  new_mode:
```
...
matter.smoke_co_alarm.self_test_request:
```
...
matter.dishwasher_alarm.reset:
  # alarms: # default: 0x00000000

# This command allows a client to request that an alarm be enabled or suppressed at the server.
matter.dishwasher_alarm.modify_enabled_alarms:
...
matter.microwave_oven_control.set_cooking_parameters:
  # cook_mode:
...
matter.microwave_oven_control.add_more_time:
  time_to_add:
```
...
matter.operational_state.pause:

# This command SHALL be supported if the device supports remotely stopping the operation.
matter.operational_state.stop:

# This command SHALL be supported if the device supports remotely starting the operation.
matter.operational_state.start:

# This command SHALL be supported if the device supports remotely resuming the operation.
matter.operational_state.resume:
```
...
```yaml
# This command SHALL be supported if the device supports remotely pausing the operation.
matter.rvc_operational_state.pause:

# This command SHALL be supported if the device supports remotely resuming the operation.
matter.rvc_operational_state.resume:
...
matter.rvc_operational_state.go_home:
```
...
matter.scenes_management.add_scene:
  group_id:
  scene_id:
  transition_time:
  scene_name:
...
matter.scenes_management.view_scene:
  group_id:
  scene_id:

# Removes the requested scene entry, corresponding to the value of the GroupID field, from its Scene Table
matter.scenes_management.remove_scene:
  group_id:
  scene_id:

# Remove all scenes, corresponding to the value of the GroupID field, from its Scene Table
matter.scenes_management.remove_all_scenes:
  group_id:

# Adds the scene entry into its Scene Table along with all extension field sets corresponding to the current state of other clusters on the same endpoint
matter.scenes_management.store_scene:
  group_id:
  scene_id:

# Set the attributes and corresponding state for each other cluster implemented on the endpoint accordingly to the resquested scene entry in the Scene Table
matter.scenes_management.recall_scene:
  group_id:
  scene_id:
  # transition_time:
...
matter.scenes_management.get_scene_membership:
  group_id:
...
matter.scenes_management.copy_scene:
...
matter.groupcast.update_group_key:
  group_id:
  key_set_id:
  # key:
...
matter.hepa_filter_monitoring.reset_condition:
```
...
```yaml
# Upon receipt, the device SHALL reset the Condition and ChangeIndicator attributes, indicating full resource availability and readiness for use, as initially configured.
matter.activated_carbon_filter_monitoring.reset_condition:
```
...
```yaml
# Upon receipt, the device SHALL reset the Condition and ChangeIndicator attributes, indicating full resource availability and readiness for use, as initially configured.
matter.water_tank_level_monitoring.reset_condition:
```
...
matter.boolean_state_configuration.suppress_alarm:
...
matter.boolean_state_configuration.enable_disable_alarm:
...
matter.valve_configurationand_control.open:
  # open_duration:
  # target_level:

# This command is used to set the valve to its closed position.
matter.valve_configurationand_control.close:
```
...
matter.water_heater_management.boost:
  boost_info:

# Allows a client to cancel an ongoing Boost operation.
matter.water_heater_management.cancel_boost:
```
...
matter.messages.present_messages_request:
  message_id:
...
matter.messages.cancel_messages_request:
  message_i_ds:
```
...
matter.device_energy_management.power_adjust_request:
  power:
  duration:
  cause: # enum: local_optimization, grid_optimization

# Allows a client to cancel an ongoing PowerAdjustmentRequest operation.
matter.device_energy_management.cancel_power_adjust_request:
...
matter.device_energy_management.start_time_adjust_request:
  requested_start_time:
  cause: # enum: local_optimization, grid_optimization

# Allows a client to temporarily pause an operation and reduce the ESAs energy demand.
matter.device_energy_management.pause_request:
  duration:
  cause: # enum: local_optimization, grid_optimization

# Allows a client to cancel the PauseRequest command and enable earlier resumption of operation.
matter.device_energy_management.resume_request:

# Allows a client to modify a Forecast within the limits allowed by the ESA.
matter.device_energy_management.modify_forecast_request:
  forecast_id:
  slot_adjustments:
  cause: # enum: local_optimization, grid_optimization

# Allows a client to ask the ESA to recompute its Forecast based on power and time constraints.
matter.device_energy_management.request_constraint_based_forecast:
  constraints:
  cause: # enum: local_optimization, grid_optimization
...
matter.device_energy_management.cancel_power_range_adjust_request:
```
...
matter.energy_evse.enable_charging:
...
```yaml
# This command is used to change device modes.
matter.energy_evse_mode.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.water_heater_mode.change_to_mode:
  new_mode:
```
...
```yaml
# This command is used to change device modes.
matter.device_energy_management_mode.change_to_mode:
  new_mode:
```
...
# This command allows a client to request that an alarm be enabled or suppressed at the server.
matter.electrical_alarm.modify_enabled_alarms:
...
matter.electrical_alarm.set_electrical_alarm_thresholds:
...
```yaml
# This command allows a client to request that an alarm be enabled or suppressed at the server.
matter.electrical_protection_alarm.modify_enabled_alarms:
...
matter.door_lock.get_week_day_schedule:
  week_day_index:
  user_index:

# Clear the specific weekly schedule or all weekly schedules for the specific user.
matter.door_lock.clear_week_day_schedule:
  week_day_index:
  user_index:

# Set a time-specific schedule ID for a specified user.
matter.door_lock.set_year_day_schedule:
  year_day_index:
  user_index:
  local_start_time:
  local_end_time:
...
matter.door_lock.get_holiday_schedule:
  holiday_index:

# Clears the holiday schedule or all holiday schedules.
matter.door_lock.clear_holiday_schedule:
  holiday_index:

# Set user into the lock.
matter.door_lock.set_user:
...
matter.door_lock.clear_aliro_reader_config:
```
...
matter.window_covering.up_or_open:
...
matter.window_covering.down_or_close:
...
matter.window_covering.stop_motion:

# Go to lift value specified
matter.window_covering.go_to_lift_value:
  lift_value:
...
matter.window_covering.go_to_lift_percentage:
  lift_percent100ths_value:

# Go to tilt value specified
matter.window_covering.go_to_tilt_value:
  tilt_value:
...
matter.window_covering.go_to_tilt_percentage:
...
matter.closure_control.calibrate:
```
...
matter.thermostat.get_weekly_schedule:
...
matter.humidistat.set_settings:
  # user_setpoint:
...
matter.color_control.move_to_hue:
  hue:
...
matter.color_control.move_to_saturation:
  saturation:
  transition_time:
  # options_mask: # bitmap: execute_if_off
  # options_override: # bitmap: execute_if_off
...
matter.color_control.move_saturation:
  move_mode: # enum: stop, up, down
  rate:
  # options_mask: # bitmap: execute_if_off
  # options_override: # bitmap: execute_if_off
...
matter.color_control.step_saturation:
  step_mode: # enum: up, down
  step_size:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.move_to_hue_and_saturation:
  hue:
  saturation:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.move_to_color:
  color_x:
  color_y:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.move_color:
  rate_x:
  rate_y:
  # options_mask: # bitmap: execute_if_off
  # options_override: # bitmap: execute_if_off
...
matter.color_control.step_color:
  step_x:
  step_y:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.move_to_color_temperature:
...
matter.color_control.enhanced_move_to_hue:
  enhanced_hue:
  direction: # enum: shortest, longest, up, down
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.enhanced_move_hue:
  move_mode: # enum: stop, up, down
  rate:
  # options_mask: # bitmap: execute_if_off
  # options_override: # bitmap: execute_if_off
...
matter.color_control.enhanced_step_hue:
  step_mode: # enum: up, down
  step_size:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.enhanced_move_to_hue_and_saturation:
  enhanced_hue:
  saturation:
  transition_time:
  # options_mask: # bitmap: execute_if_off
...
matter.color_control.color_loop_set:
...
matter.color_control.step_color_temperature:
  step_mode: # enum: up, down
  step_size:
  transition_time:
  color_temperature_minimum_mireds:
...
matter.dynamic_lighting.start_effect:
  effect_id:
  speed:
...
matter.dynamic_lighting.stop_effect:
```
...
matter.proximity_ranging.start_ranging_request:
...
matter.proximity_ranging.stop_ranging_request:
  session_id:
```
...
matter.network_identity_management.import_admin_secret:
  network_administrator_shared_secret:

# Exports the current Network Administrator Shared Secret.
matter.network_identity_management.export_admin_secret:
```
...
matter.wi_fi_network_management.network_passphrase_request:
```
...
matter.thread_border_router_management.get_active_dataset_request:

# This command SHALL be used to request the pending dataset of the Thread network to which the border router is connected.
matter.thread_border_router_management.get_pending_dataset_request:
...
matter.thread_border_router_management.set_active_dataset_request:
  active_dataset:
  # breadcrumb:
...
matter.thread_border_router_management.set_pending_dataset_request:
  pending_dataset:
```
...
matter.channel.get_program_guide:
  # start_time:
  # end_time:
  # channel_list:
  # page_token:
...
matter.channel.cancel_record_program:
  program_identifier:
  should_record_series:
  external_id_list:
  data:
...
matter.target_navigator.navigate_target:
  target:
  # data:
```
...
matter.media_playback.play:

# Upon receipt, this SHALL pause media.
matter.media_playback.pause:
...
matter.media_playback.stop:

# Upon receipt, this SHALL Start Over with the current media playback item.
matter.media_playback.start_over:
...
matter.media_playback.previous:
...
matter.media_playback.skip_backward:
  delta_position_milliseconds:

# Upon receipt, this SHALL Skip backward in the media by the given number of seconds, using the data as follows:
matter.media_playback.seek:
  position:
...
matter.media_playback.activate_audio_track:
  track_id:
  audio_output_index:
...
matter.content_launcher.launch_content:
  search:
  auto_play:
  # data:
...
matter.content_launcher.launch_url:
  content_url:
  # display_string:
...
matter.audio_output.select_output:
  index:

# Upon receipt, this SHALL rename the output at a specific index in the Output List.
matter.audio_output.rename_output:
  index:
  name:
```
...
matter.account_login.get_setup_pin:
...
matter.content_app_observer.content_app_message:
  # data:
  encoding_hint:
```
...
matter.camera_av_stream_management.audio_stream_allocate:
...
matter.camera_av_stream_management.audio_stream_deallocate:
  audio_stream_id:

# This command SHALL allocate a video stream on the camera and return an allocated video stream identifier.
matter.camera_av_stream_management.video_stream_allocate:
  stream_usage: # enum: internal, recording, analysis, live_view
...
matter.camera_av_settings_user_level_management.mptz_set_position:
  # pan:
  # tilt:
  # zoom:

# This command SHALL move the camera by the delta values relative to the currently defined position.
matter.camera_av_settings_user_level_management.mptz_relative_move:
  # pan_delta: # default: 0
...
matter.camera_av_settings_user_level_management.mptz_move_to_preset:
  preset_id:

# This command allows creating a new preset or updating the values of an existing one.
matter.camera_av_settings_user_level_management.mptz_save_preset:
  # preset_id:
  name:

# This command SHALL remove a preset entry from the PresetMptzTable.
matter.camera_av_settings_user_level_management.mptz_remove_preset:
  preset_id:

# This command allows for setting the digital viewport for a specific Video Stream.
matter.camera_av_settings_user_level_management.dptz_set_viewport:
  video_stream_id:
  viewport:

# This command SHALL change the per stream viewport by the amount specified in a relative fashion.
matter.camera_av_settings_user_level_management.dptz_relative_move:
  video_stream_id:
  # delta_x: # default: 0
  # delta_y: # default: 0
  # zoom_delta: # default: 0
...
matter.web_rtc_transport_provider.solicit_offer:
  stream_usage: # enum: internal, recording, analysis, live_view
...
matter.web_rtc_transport_provider.provide_offer:
  web_rtc_session_id:
  sdp:
  stream_usage: # enum: internal, recording, analysis, live_view
  originating_endpoint_id:
...
matter.web_rtc_transport_provider.provide_answer:
  web_rtc_session_id:
  sdp:
...
matter.web_rtc_transport_provider.provide_ice_candidates:
  web_rtc_session_id:
  ice_candidates:

# This command instructs the stream provider to end the WebRTC session.
matter.web_rtc_transport_provider.end_session:
  web_rtc_session_id:
...
matter.web_rtc_transport_requestor.offer:
  web_rtc_session_id:
  sdp:
  # ice_servers:
  # ice_transport_policy:
...
matter.web_rtc_transport_requestor.answer:
  web_rtc_session_id:
  sdp:
...
matter.push_av_stream_transport.set_transport_status:
  connection_id:
...
matter.push_av_stream_transport.manually_trigger_transport:
  connection_id:
...
matter.push_av_stream_transport.find_transport:
  connection_id:
```
...
matter.av_analysis.enable_context_triggers:
  context_triggers:
...
matter.av_analysis.disable_context_triggers:
  context_triggers:
...
matter.av_analysis.establish_analysis_stream:
  node_id:
...
matter.av_analysis.activate_analysis_stream:
  analysis_stream_id:
  # web_rtc_endpoint_id:
  # push_av_endpoint_id:
...
matter.commodity_tariff.get_tariff_component:
  tariff_component_id:
...
matter.commodity_tariff.get_day_entry:
  day_entry_id:
```
...
matter.commissioner_control.request_commissioning_approval:
  request_id:
  vendor_id:
  product_id:
  # label:
...
matter.commissioner_control.commission_node:
  request_id:
...
matter.joint_fabric_datastore.update_group:
  group_id:
  friendly_name:
  group_key_set_id:
  group_cat:
...
matter.joint_fabric_datastore.update_endpoint_for_node:
  endpoint_id:
  node_id:
  friendly_name:
...
matter.joint_fabric_datastore.add_group_id_to_endpoint_for_node:
  node_id:
  endpoint_id:
  group_id:
...
matter.joint_fabric_datastore.remove_group_id_from_endpoint_for_node:
  node_id:
  endpoint_id:
  group_id:
...
matter.joint_fabric_datastore.add_binding_to_endpoint_for_node:
  node_id:
  endpoint_id:
  binding:
...
matter.joint_fabric_administrator.icaccsr_request:
...
matter.joint_fabric_administrator.add_icac:
  icac_value:
...
matter.joint_fabric_administrator.open_joint_commissioning_window:
  commissioning_timeout:
  pake_passcode_verifier:
  discriminator:
  iterations:
...
matter.joint_fabric_administrator.transfer_anchor_request:
...
matter.joint_fabric_administrator.transfer_anchor_complete:

# This command SHALL be used for communicating to client the endpoint that holds the Joint Fabric Administrator Cluster.
matter.joint_fabric_administrator.announce_joint_fabric_administrator:
  endpoint_id:
```
...
matter.tls_certificate_management.provision_root_certificate:
  certificate:
  caid:
...
matter.tls_certificate_management.find_root_certificate:
  caid:

# This command SHALL return the CAID for the passed in fingerprint.
matter.tls_certificate_management.lookup_root_certificate:
  fingerprint:
...
matter.tls_certificate_management.remove_root_certificate:
  caid:
...
matter.tls_certificate_management.client_csr:
  nonce:
  ccdid:
...
matter.tls_certificate_management.provision_client_certificate:
  ccdid:
  client_certificate:
...
matter.tls_certificate_management.find_client_certificate:
  ccdid:

# This command SHALL return the CCDID for the passed in Fingerprint.
matter.tls_certificate_management.lookup_client_certificate:
  fingerprint:

# This command SHALL be used to request the Node removes all stored information for the provided CCDID.
matter.tls_certificate_management.remove_client_certificate:
  ccdid:
```
...
matter.tls_client_management.provision_endpoint:
  hostname:
  port:
  caid:
  ccdid:
...
matter.tls_client_management.find_endpoint:
  endpoint_id:

# This command is used to remove a TLS Endpoint by its ID.
matter.tls_client_management.remove_endpoint:
  endpoint_id:
```
...
matter.unit_testing.test_struct_argument_request:
  arg1:

# Command that takes an argument which is nested struct. The response
# echoes the 'b' field of ar1.c.
matter.unit_testing.test_nested_struct_argument_request:
  arg1:
...
matter.unit_testing.test_list_struct_argument_request:
  arg1:
...
matter.unit_testing.test_list_int8_u_argument_request:
  arg1:
...
# otherwise.
matter.unit_testing.test_nested_struct_list_argument_request:
  arg1:
...
matter.unit_testing.test_nullable_optional_request:
  # arg1:
...
# their state was.
matter.unit_testing.test_complex_nullable_optional_request:
  nullable_int:
  # optional_int:
...
# the struct back.
matter.unit_testing.simple_struct_echo_request:
  arg1:

# Command that just responds with a success status if the timed invoke
# conditions are met.
matter.unit_testing.timed_invoke_request:
...
matter.unit_testing.test_simple_optional_argument_request:
  # arg1:
...
matter.unit_testing.test_emit_test_event_request:
  arg1:
  arg2: # enum: unspecified, value_a, value_b, value_c
  arg3:
...
matter.unit_testing.test_second_batch_helper_request:
  sleep_before_response_time_ms:
  size_of_response_buffer:
  fill_character:
...
matter.unit_testing.test_check_command_flags:

# Command having a different MEI vendor ID than the cluster. Also emits TestDifferentVendorMeiEvent.
matter.unit_testing.test_different_vendor_mei_request:
  arg1:
```
...
matter.fault_injection.fail_at_fault:
...
matter.fault_injection.fail_randomly_at_fault:
  type: # enum: unspecified, system_fault, inet_fault, chip_fault, cert_fault
  id:
  percentage:
```
